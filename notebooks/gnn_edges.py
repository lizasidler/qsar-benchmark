...
class GatedEdgeConv(nn.Module):
	...
	def __init__(self, hidden_dim, hidden_dim_3, output_dim):
		...
		self.layer_gnn_h = nn.Sequential(nn.Linear((hidden_dim+33)*3, hidden_dim_3), nn.BatchNorm1d(hidden_dim_3), nn.SiLU())
...
class NNConv(nn.Module):
	...
	def __init__(self, hidden_dim_1, hidden_dim_2, hidden_dim_3, output_dim):
		...
		self.edge_nn_2 = nn.Sequential(
	    	nn.Linear(4, 32),
	    	nn.ReLU(),
	    	nn.Linear(32, hidden_dim_1 * hidden_dim_2)
				)

		self.conv2 = gnn.Sequential('x, edge_index, edge_attr', [
			(gnn.NNConv(hidden_dim_1, hidden_dim_2, self.edge_nn_2, aggr='add'), 'x, edge_index, edge_attr -> x'),
			(nn.LayerNorm(hidden_dim_2), 'x -> x'),
			(nn.SiLU(), 'x -> x'),
			])


		self.layer_gnn_h = nn.Sequential(nn.Linear((hidden_dim_2+33)*3, hidden_dim_3), nn.BatchNorm1d(hidden_dim_3), nn.SiLU())
		self.layer_h_out = nn.Linear(hidden_dim_3, output_dim)
...
class GINE(nn.Module):
	...
	def __init__(self, hidden_dim, output_dim):
		...
		self.conv = gnn.Sequential('x, edge_index, edge_attr', [
			(gnn.GINEConv(nn.Sequential(nn.Linear(26, hidden_dim), nn.LayerNorm(hidden_dim), nn.SiLU()), edge_dim=4), 'x, edge_index, edge_attr -> x'),
			(nn.LayerNorm(hidden_dim), 'x -> x'),
			(nn.SiLU(), 'x -> x'),
			])

		self.conv_gate = gnn.Sequential('x, edge_index', [
			(gnn.GatedGraphConv(hidden_dim, 2), 'x, edge_index -> x'),
			(nn.LayerNorm(hidden_dim), 'x -> x'),
			(nn.SiLU(), 'x -> x'),
			])

		self.layer_gnn_h = nn.Sequential(nn.Linear((hidden_dim*2+26)*3, 64), nn.BatchNorm1d(64), nn.SiLU(), nn.Dropout(0.3))
	...
	def forward(self, batch):
		...
		x0 = self.conv(x, edge_index, edge_attr)
		x1 = self.conv_gate(x, edge_index)

		x_all = torch.cat([x, x0, x1], dim=-1)
		x_mean = gnn.global_mean_pool(x_all, _batch)
		x_max = gnn.global_max_pool(x_all, _batch)
		x_add = gnn.global_add_pool(x_all, _batch)
...
class Attention(nn.Module):
	...
	def __init__(self, hidden_dim, output_dim):
		...
		self.layer_gnn_h = nn.Sequential(nn.Linear((hidden_dim+26)*3, 64), nn.BatchNorm1d(64), nn.SiLU(), nn.Dropout(0.3))
		self.layer_h_out = nn.Linear(64, output_dim)
